...
public:
	...
	//doesn't compile because there's no way to compare array sizes, also unneccessary because value tree comparisons
	/*inline bool compare(Piano::Ptr p)
	{
		if (items.size() != p->getItems.size()) return false;
		for (int i = 0; i < items.size(); i++)
		{
			if (items[i] != p->getItems()[i]) return false;
			if (items[i]->getConnections().size() != p->getItems()[i]->getConnections().size()) return false;
			for (int j = 0; j < items[i]->getConnections().size(); j++)
			{
				if (items[i]->getConnections()[j] != p->getItems()[i]->getConnections()[j]) return false;
			}
		}
		return true;
	}*/
	/*
	//currently makes 1-3 keymaps, each with a direct, tuning, nostalgic, synchronic, and tempo prepration, will refine later
	inline void randomize()
	{
		clear();
		BKSynthesiser* dummy;
		GeneralSettings g();
		int numKeymaps = (int)(rand() / RAND_MAX * 3) + 1; //1-3 keymaps
		for (int i = 0; i < numKeymaps; i++)
		{
			Keymap::Ptr kp;
			kp->randomize();
			addPreparationMap(kp);

			TuningPreparation::Ptr tPrep = new TuningPreparation();
			tPrep->randomize();
			Tuning::Ptr t = new Tuning(tPrep, 1);
			TuningProcessor::Ptr tProc = new TuningProcessor(t);
			addTuningProcessor(1);
			
			DirectPreparation::Ptr dPrep = new DirectPreparation();
			dPrep->randomize();
			Direct::Ptr d = new Direct(dPrep, 2);
			DirectProcessor::Ptr dProc = new DirectProcessor(d, t, dummy, dummy, dummy);
			addDirectProcessor(2);

			TempoPreparation::Ptr mPrep = new TempoPreparation();
			mPrep->randomize();
			Tempo::Ptr m = new Tempo(mPrep, 3);
			TempoProcessor::Ptr mProc = new TempoProcessor(m);
			addTempoProcessor(3);

			SynchronicPreparation::Ptr sPrep = new SynchronicPreparation();
			sPrep->randomize();
			Synchronic::Ptr s = new Synchronic(sPrep, 4);
			SynchronicProcessor::Ptr sProc = new SynchronicProcessor(s, t, m, dummy, g);
			addSynchronicProcessor(4);

			NostalgicPreparation::Ptr nPrep = new NostalgicPreparation();
			nPrep->randomize();
			Nostalgic::Ptr n = new Nostalgic(nPrep, 5);
			NostalgicProcessor::Ptr nProc = new NostalgicProcessor(n, tProc, sProc, dummy);
			addNostalgicProcessor(5);

			
			//randomly chooses whether to link synchronic with keymap, tuning, or nostalgic
			float linkType = (float)rand() / RAND_MAX;
			if (linkType > 0.66) linkPreparationWithKeymap(PreparationTypeSynchronic, 3, kp->getId());
			else if (linkType > 0.33) linkPreparationWithTuning(PreparationTypeSynchronic, 3, tp);
			else linkNostalgicWithSynchronic(np, sp);


			linkPreparationWithKeymap(PreparationTypeDirect, d.getId(), kp->getId());

			linkPreparationWithTuning(PreparationTypeDirect, d.getId(), );

			//randomly chooses whether to link nostalgic with keymap or tuning
			if (rand() / RAND_MAX > 0.5) linkPreparationWithKeymap(PreparationTypeNostalgic, 2, kp->getId());
			else linkPreparationWithTuning(PreparationTypeNostalgic, 2, tp);

			linkSynchronicWithTempo(sp, mp);
