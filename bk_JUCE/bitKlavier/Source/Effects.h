...
public:
    ...
    EffectProcessor(GeneralSettings::Ptr bGeneral, EffectType type, TempoProcessor::Ptr tempo) : effectActive(true), keymaps(Keymap::PtrArr()), general(bGeneral), type(type), tempo(tempo)
    {
        sampleRate = 48000;
    }
    ...
    virtual void prepareToPlay(double sr)
    {
        sampleRate = sr;
    };
