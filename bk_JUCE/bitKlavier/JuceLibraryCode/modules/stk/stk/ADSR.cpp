...
StkFloat ADSR :: getReleaseTime( void )
{
    return releaseTime_;
}
...
void AHDSR :: sampleRateChanged( StkFloat newRate, StkFloat oldRate )
{
    if ( !ignoreSampleRateChange_ ) {
        attackRate_ = oldRate * attackRate_ / newRate;
...
void AHDSR :: setAttackRate( StkFloat rate )
{
    if ( rate < 0.0 ) {
        oStream_ << "ADSR::setAttackRate: argument must be >= 0.0!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setAttackTarget( StkFloat target )
{
    if ( target < 0.0 ) {
        oStream_ << "ADSR::setAttackTarget: negative target not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setHoldTime( StkFloat time )
{
    if ( time < 0.0 ){
        oStream_ << "ADSR::setHoldTime: negative time not allowed!";
        handleError( StkError::WARNING ); return;
    }
    
    holdSamples_ = time * Stk::sampleRate();
}

void AHDSR :: setDecayRate( StkFloat rate )
{
    if ( rate < 0.0 ) {
        oStream_ << "ADSR::setDecayRate: negative rates not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setSustainLevel( StkFloat level )
{
    if ( level < 0.0 ) {
        oStream_ << "ADSR::setSustainLevel: negative level not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setReleaseRate( StkFloat rate )
{
    if ( rate < 0.0 ) {
        oStream_ << "ADSR::setReleaseRate: negative rates not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setAttackTime( StkFloat time )
{
    if ( time <= 0.0 ) {
        oStream_ << "ADSR::setAttackTime: negative or zero times not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setDecayTime( StkFloat time )
{
    if ( time <= 0.0 ) {
        oStream_ << "ADSR::setDecayTime: negative or zero times not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setAllTimes( StkFloat aTime, StkFloat hTime, StkFloat dTime, StkFloat sLevel, StkFloat rTime )
{
    this->setSustainLevel( sLevel );
...
void AHDSR :: setTarget( StkFloat target )
{
    if ( target < 0.0 ) {
        oStream_ << "ADSR::setTarget: negative target not allowed!";
        handleError( StkError::WARNING ); return;
...
void AHDSR :: setValue( StkFloat value )
{
    state_ = SUSTAIN;
    target_ = value;
    value_ = value;
